...
def RunSingleInference(
    ...
    Resolution: int,
    MinScoreForNms: float = DEFAULT_INFER_MIN_SCORE,
    ...
    ScoresTensor = R["scores"].detach().cpu()
    BoxesTensor = R["boxes"].detach().cpu()
    LabelsTensor = R["labels"].detach().cpu()

    # 1) 超低阈值预筛选 (尽量保留预测)
    KeepMin = ScoresTensor >= MinScoreForNms
    ScoresTensor = ScoresTensor[KeepMin]
    BoxesTensor = BoxesTensor[KeepMin]
    LabelsTensor = LabelsTensor[KeepMin]

    # 2) 按类别执行 NMS
    if ScoresTensor.numel() > 0:
        KeepIdxAll = []
        for Label in LabelsTensor.unique():
            LabelMask = LabelsTensor == Label
            LabelIndices = torch.nonzero(LabelMask, as_tuple=False).squeeze(1)
            LabelKeep = tvops.nms(
                BoxesTensor[LabelMask],
                ScoresTensor[LabelMask],
                NmsIouThreshold,
            )
            KeepIdxAll.append(LabelIndices[LabelKeep])

        if KeepIdxAll:
            KeepIdx = torch.cat(KeepIdxAll)
            SortOrder = torch.argsort(ScoresTensor[KeepIdx], descending=True)
            KeepIdx = KeepIdx[SortOrder]
            ScoresTensor = ScoresTensor[KeepIdx]
            BoxesTensor = BoxesTensor[KeepIdx]
            LabelsTensor = LabelsTensor[KeepIdx]

    return {
        "scores": ScoresTensor.numpy(),
        "boxes": BoxesTensor.numpy(),
        "labels": LabelsTensor.numpy(),
...
def Evaluate(
    ...
    InferenceMinScore: float = DEFAULT_INFER_MIN_SCORE,
    NmsIouThreshold: float = DEFAULT_NMS_IOU,
    Device: str = "cuda",
) -> dict:
    """完整评估。
    ...
    for Sample in Pbar:
        ...
        Result = RunSingleInference(
            ...
            ImgPil=ImgPil,
            ExemplarBoxes=ExBoxes,
            Resolution=Resolution,
            MinScoreForNms=InferenceMinScore,
    ...
    Stats = {
        ...
        "infer_min_score": InferenceMinScore,
        "nms_iou": NmsIouThreshold,
...
def ParseArgs() -> argparse.Namespace:
    ...
    P.add_argument(
        "--infer_min_score", type=float, default=DEFAULT_INFER_MIN_SCORE,
        help="推理阶段的最低置信度阈值 (NMS 前预筛选), 默认接近 0",
    )
    P.add_argument(
        "--nms_iou", type=float, default=DEFAULT_NMS_IOU,
        help="NMS IoU 阈值",
    )
    P.add_argument(
...
if __name__ == "__main__":
    ...
    print(f"  Infer min score: {Args.infer_min_score}")
    ...
    Stats = Evaluate(
        ...
        InferenceMinScore=Args.infer_min_score,
        NmsIouThreshold=Args.nms_iou,
        Device=Args.device,
    )
