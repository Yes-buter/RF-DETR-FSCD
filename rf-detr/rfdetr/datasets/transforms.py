...
def crop(image, target, region):
    ...
    # FSCD: exemplar_boxes 同步 crop 变换
    if "exemplar_boxes" in target:
        ex_boxes = target["exemplar_boxes"]
        max_size = torch.as_tensor([w, h], dtype=torch.float32)
        cropped_ex = ex_boxes - torch.as_tensor([j, i, j, i])
        cropped_ex = torch.min(cropped_ex.reshape(-1, 2, 2), max_size)
        cropped_ex = cropped_ex.clamp(min=0)
        target["exemplar_boxes"] = cropped_ex.reshape(-1, 4)

    # FSCD: points 同步 crop 变换
    if "points" in target and target["points"].numel() > 0:
        pts = target["points"] - torch.as_tensor([j, i], dtype=torch.float32)
        # 保留在 crop 区域内的点
        valid = (pts[:, 0] >= 0) & (pts[:, 0] < w) & (pts[:, 1] >= 0) & (pts[:, 1] < h)
        target["points"] = pts[valid]
...
def hflip(image, target):
    ...
    # FSCD: exemplar_boxes 同步水平翻转
    if "exemplar_boxes" in target:
        ex_boxes = target["exemplar_boxes"]
        ex_boxes = ex_boxes[:, [2, 1, 0, 3]] * torch.as_tensor([-1, 1, -1, 1]) + torch.as_tensor([w, 0, w, 0])
        target["exemplar_boxes"] = ex_boxes

    # FSCD: points 同步水平翻转 (x → w - x)
    if "points" in target and target["points"].numel() > 0:
...
class SquareResize(object):
    ...
    def __call__(self, img, target=None):
        ...
        # FSCD: exemplar_boxes 同步缩放
        if "exemplar_boxes" in target:
            ex_boxes = target["exemplar_boxes"]
            target["exemplar_boxes"] = ex_boxes * torch.as_tensor(
                [ratio_width, ratio_height, ratio_width, ratio_height])

        # FSCD: points 同步缩放
        if "points" in target and target["points"].numel() > 0:
            target["points"] = target["points"] * torch.as_tensor(
                [ratio_width, ratio_height], dtype=torch.float32)
...
class PhotometricDistort(object):
    """颜色抖动增强 (仅修改像素, 不影响 target)。

    对 PIL Image 随机调整亮度/对比度/饱和度/色相。
    """
    def __init__(
        self,
